  ...
  <div id="app" style = "margin: -1% -0.6%">
    ...
    <div class="UploadDIV">
      <div class="UploadInnerDIV">
        <el-upload
          action="http://127.0.0.1:8000/upload_image"
    ...
    <div style = "padding-top: 50px; padding-bottom: 10px; font-weight: bold; font-size: 20px">
      <span>请上传PNG格式其他图片</span>
    </div>
    <div class="UploadDIV">
      <div class="UploadInnerDIV">
        <el-upload
          action="http://127.0.0.1:8000/upload_others"
          list-type="picture-card"
          :on-preview="handlePictureCardPreview1"
          :on-remove="handleRemove"
          style = "padding: 2%">
    ...
    <div style = "padding-top: 50px">
      <el-button type="primary" @click="detect">寻找最相似的图片</el-button>
    </div>
    <div style = "padding-top: 50px; padding-bottom: 10px; font-weight: bold; font-size: 20px">
      <span>最相似的图片</span>
    </div>
    <div>
      <img src = "../assets/Similarity/similar.png" style = "width: 30%; padding: 2%">
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    detect () {
      this.$api.post('/detect_similarity').then(response => {
        this.image = true;
        this.$message.success('图片已找到！');
      })
    }
  }
}
