        ...
        <el-col :span="6" v-for="(item, index) in softmusic" style = "margin: 2% auto">
          <div class="img_div" style="height: 230px; padding: 20px">
            <div>
              ...
              <img :src="require('../assets/soft/'+softimage[index])" style="width: 100%; height: 200px; border-radius:10px 10px;">
            </div>
            <audio controls="controls" style="width: 300px">
              <source :src="require('../assets/soft/'+softmusic[index])">
            </audio>
          </div>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    GetSoftMusicUrl () {
      this.$api.post('/get_soft_music').then(response => {
        this.softmusic = response.data
      })
    },
    GetSoftImageUrl () {
      this.$api.post('/get_soft_image').then(response => {
        this.softimage = response.data
        //console.log(this.softimage)
      })
    },
    GetSoftTxt () {
      this.$api.post('/get_soft_txt').then(response => {
        this.softtxt = response.data
      })
    },
    show (index) {
      return this.softtxt[index]
    }
  },
  mounted () {
    this.GetSoftTxt()
